...
import { ChevronDown, ChevronRight, Search, Cloud } from "lucide-react";
...
function CloudSidebar() {
  ...
  const filtered = categories
    .filter((cat) => cat.name === "Cloud Infrastructure")
    .map((cat) => ({
      ...cat,
    ...
    <div className="w-[280px] border-r border-border bg-card flex flex-col shrink-0">
      <div className="p-3 border-b border-border">
        <div className="relative">
          ...
          <Input placeholder="Search cloud labs..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs bg-secondary border-border font-mono-terminal" />
        </div>
      </div>
...
function CloudLabInner() {
  const { selectLab } = useWorkspace();
  useEffect(() => {
    const cloudLabs = labs.filter((l) => l.category === "Cloud Infrastructure");
    if (cloudLabs.length > 0) selectLab(cloudLabs[0]);
  }, []);
...
export default function CloudLabPage() {
  return <WorkspaceProvider><CloudLabInner /></WorkspaceProvider>;
}
