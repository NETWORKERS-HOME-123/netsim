...
function PythonSecOpsSidebar() {
  ...
  const filtered = categories
    .filter((cat) => cat.name === "Python Security")
    .map((cat) => ({
      ...cat,
    ...
    <div className="w-[280px] border-r border-border bg-card flex flex-col shrink-0">
      <div className="p-3 border-b border-border">
        <div className="relative">
          ...
          <Input placeholder="Search Python SecOps labs..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs bg-secondary border-border font-mono-terminal" />
        </div>
      </div>
...
function PythonSecOpsLabInner() {
  const { selectLab } = useWorkspace();
  useEffect(() => {
    const pySecLabs = labs.filter((l) => l.category === "Python Security");
    if (pySecLabs.length > 0) selectLab(pySecLabs[0]);
  }, []);
...
export default function PythonSecOpsLabPage() {
  return <WorkspaceProvider><PythonSecOpsLabInner /></WorkspaceProvider>;
}
