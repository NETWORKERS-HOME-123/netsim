    ...
    <div className="w-[280px] border-r border-border bg-card flex flex-col shrink-0">
      <div className="p-3 border-b border-border">
        <div className="relative">
          ...
          <Input placeholder="Search security labs..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs bg-secondary border-border font-mono-terminal" />
        </div>
      </div>
...
function SecurityLabInner() {
  const { selectLab } = useWorkspace();
  useEffect(() => {
    const secLabs = labs.filter((l) => l.category === "Security");
    if (secLabs.length > 0) selectLab(secLabs[0]);
  }, []);
...
export default function SecurityLabPage() {
  return <WorkspaceProvider><SecurityLabInner /></WorkspaceProvider>;
}
