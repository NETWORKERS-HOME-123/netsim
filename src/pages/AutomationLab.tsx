...
import { ChevronDown, ChevronRight, Search, Code, Cloud } from "lucide-react";
import { cn } from "@/lib/utils";
import type { Lab, LabMode } from "@/data/labs";

const automationModes: LabMode[] = ["Python Lab", "Terraform Lab"];
...
function AutomationSidebar() {
  ...
  const automationCategories = ["Python Networking", "Terraform"];

  const filteredCategories = categories
    .filter((cat) => automationCategories.includes(cat.name))
    .map((cat) => {
    ...
    <div className="w-[280px] border-r border-border bg-card flex flex-col shrink-0">
      <div className="p-3 border-b border-border">
        <div className="relative">
          ...
          <Input
            placeholder="Search automation labs..."
            value={search}
              ...
              {isOpen && (
                <div className="ml-2 border-l border-border">
                  {cat.filteredLabs.map((lab) => {
                    const Icon = modeIcons[lab.mode] || Code;
...
function AutomationLabInner() {
  const { selectLab } = useWorkspace();

  // Select first Python lab on mount
  useEffect(() => {
    const pythonLabs = labs.filter((l) => l.mode === "Python Lab");
    if (pythonLabs.length > 0) selectLab(pythonLabs[0]);
  }, []);
