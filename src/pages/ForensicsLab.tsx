...
import { ChevronDown, ChevronRight, Search as SearchIcon, FileSearch } from "lucide-react";
import { cn } from "@/lib/utils";

function ForensicsSidebar() {
    ...
    <div className="w-[280px] border-r border-border bg-card flex flex-col shrink-0">
      <div className="p-3 border-b border-border">
        <div className="relative">
          ...
          <Input placeholder="Search forensics labs..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs bg-secondary border-border font-mono-terminal" />
        </div>
      </div>
...
function ForensicsLabInner() {
  const { selectLab } = useWorkspace();
  useEffect(() => {
    const fLabs = labs.filter((l) => l.category === "Forensics");
    if (fLabs.length > 0) selectLab(fLabs[0]);
  }, []);
...
export default function ForensicsLabPage() {
  return <WorkspaceProvider><ForensicsLabInner /></WorkspaceProvider>;
}
